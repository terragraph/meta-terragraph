/* Sourced from https://github.com/seanmiddleditch/libtelnet */
 ...
 */
telnet_error_t
_init_zlib(telnet_t* telnet, int deflate, int err_fatal) {
  z_stream* z;
  int rs;
...
static INLINE telnet_rfc1143_t
...
static INLINE void
_set_rfc1143(telnet_t* telnet, unsigned char telopt, char us, char him) {
  ...
  for (i = 0; i != telnet->q_cnt; ++i) {
    if (telnet->q[i].telopt == telopt) {
      ...
      if (telopt != TELNET_TELOPT_BINARY)
        return;
      telnet->flags &=
          ~(TELNET_FLAG_TRANSMIT_BINARY | TELNET_FLAG_RECEIVE_BINARY);
      if (us == Q_YES)
  ...
  if (telnet->q_cnt >= telnet->q_size) {
    ...
    memset(
        &qtmp[telnet->q_size],
        0,
        sizeof(telnet_rfc1143_t) * Q_BUFFER_GROWTH_QUANTUM);
    telnet->q = qtmp;
...
static int
_environ_telnet(
    telnet_t* telnet, unsigned char type, char* buffer, size_t size) {
  telnet_event_t ev;
  ...
  if ((unsigned)buffer[0] != TELNET_ENVIRON_SEND &&
    ...
    _error(
        ...
        TELNET_EPROTOCOL,
        0,
        "telopt %d subneg has invalid command",
        type);
    return 0;
  }
  ...
  if ((values = (struct telnet_environ_t*)calloc(
           count, sizeof(struct telnet_environ_t))) == 0) {
    _error(
        telnet,
        __LINE__,
        __func__,
...
static int
_mssp_telnet(telnet_t* telnet, char* buffer, size_t size) {
  telnet_event_t ev;
...
static int
_zmp_telnet(telnet_t* telnet, const char* buffer, size_t size) {
  telnet_event_t ev;
  char** argv;
  const char* c;
  size_t i, argc;
  ...
  switch (telnet->sb_telopt) {
    ...
    case TELNET_TELOPT_NEW_ENVIRON:
      return _environ_telnet(
          telnet, telnet->sb_telopt, telnet->buffer, telnet->buffer_pos);
      ...
      case TELNET_STATE_DATA:
        ...
        if (byte == TELNET_IAC) {
          ...
          telnet->state = TELNET_STATE_IAC;
            ...
            byte == '\r' && (telnet->flags & TELNET_FLAG_NVT_EOL) &&
            !(telnet->flags & TELNET_FLAG_RECEIVE_BINARY)) {
          if (i != start) {
            ev.type = TELNET_EV_DATA;
            ev.data.buffer = buffer + start;
      ...
      case TELNET_STATE_SB_DATA:
        ...
        if (byte == TELNET_IAC) {
          telnet->state = TELNET_STATE_SB_DATA_IAC;
            ...
            telnet->sb_telopt == TELNET_TELOPT_COMPRESS &&
            byte == TELNET_WILL) {
    ...
    while (telnet->z->avail_in > 0 || telnet->z->avail_out == 0) {
      ...
      if (rs == Z_OK || rs == Z_STREAM_END)
        _process(
            telnet,
            inflate_buffer,
            sizeof(inflate_buffer) - telnet->z->avail_out);
      else
        _error(
            telnet,
            __LINE__,
...
telnet_subnegotiation(
    telnet_t* telnet, unsigned char telopt, const char* buffer, size_t size) {
...
void
telnet_begin_compress2(telnet_t* telnet) {
#if defined(HAVE_ZLIB)
...
void
telnet_begin_newenviron(telnet_t* telnet, unsigned char cmd) {
...
void
telnet_newenviron_value(
    telnet_t* telnet, unsigned char type, const char* string) {
...
void
telnet_ttype_send(telnet_t* telnet) {
  static const unsigned char SEND[] = {TELNET_IAC,
                                       TELNET_SB,
                                       TELNET_TELOPT_TTYPE,
                                       TELNET_TTYPE_SEND,
                                       TELNET_IAC,
...
void
telnet_ttype_is(telnet_t* telnet, const char* ttype) {
...
/* send ZMP data */
void
telnet_send_zmp(telnet_t* telnet, size_t argc, const char** argv) {
  size_t i;
...
void
telnet_send_vzmpv(telnet_t* telnet, va_list va) {
  const char* arg;
...
void
telnet_send_zmpv(telnet_t* telnet, ...) {
  va_list va;
...
/* begin a ZMP command */
void
telnet_begin_zmp(telnet_t* telnet, const char* cmd) {
  telnet_begin_sb(telnet, TELNET_TELOPT_ZMP);
...
/* send a ZMP argument */
void
telnet_zmp_arg(telnet_t* telnet, const char* arg) {
